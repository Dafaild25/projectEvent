<div class="container my-3">
        <div class="card shadow-lg">
			...
			<div class="card-body">
    			<form id="registrationForm" method="post" action="<?php echo isset($registration)? site_url('Registrations/update'):site_url('Registrations/save');?>">
        		<div class="row g-3">
						...
						<div class="col-md-6">
							<select name="asistent_id" id="asistent_id" class="form-select">
								...
								<?php foreach ($asistents as $asistent): ?>
									<option 
										value="<?= $asistent->id; ?>" 
										<?= isset($registration) && $registration->asistent_id == $asistent->id ? 'selected' : ''; ?>>
						...
						<div class="col-md-6">
							<select name="party_id" id="party_id" class="form-control">
								...
								<?php foreach ($partys as $party): ?>
									<option 
										value="<?= $party->id; ?>" 
										<?= isset($registration) && $registration->party_id == $party->id ? 'selected' : ''; ?>>
										<?= $party->name; ?>
									</option>
								<?php endforeach; ?>
							</select>
						...
						<?php if (!empty($registrations)) { ?>
							<?php foreach ($registrations as $registration) { ?>
								<tr>
									...
									<td>
										<a href="<?php echo site_url('Registrations/selectRegistration/' .$registration->id);?>" class="btn btn-primary"> 
											<i class="fas fa-pen"></i>
										</a>
										<a href="<?php echo site_url('Registrations/delete/' . $registration->id); ?>" class="btn btn-danger">
